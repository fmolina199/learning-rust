...
async fn main() {
	...
	data_channel.on_open(Box::new(
		move || {
			...
			Box::pin(async move {
				...
				let time_str = format!("{:?}", &Instant::now());
				let encoded: Vec<u8> = match bincode::encode_to_vec(&time_str, config) {
					Ok(p) => p,
					Err(e) => panic!("{}", e),
				};
	...
	data_channel.on_message(Box::new(
		move |msg: DataChannelMessage| {
			...
			let config = config::standard();
			let (decoded, _len): (model::Chunk, usize) = match bincode::decode_from_slice(&msg.data, config) {
				Ok(p) => p,
				Err(e) => panic!("{}", e),
			};
			log::info!("Message from DataChannel '{}': '{:?}'", d_label, decoded);
			Box::pin(async {})
		}
	));
	...
	let payload = match peer_connection.local_description().await {
		Some(local_desc) => model::ConnectRequest {
			id: rand::random::<u64>(),
